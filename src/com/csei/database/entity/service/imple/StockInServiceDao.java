...
import com.csei.database.entity.service.StockInService;

public class StockInServiceDao implements StockInService {
	private DBOptions dbOptions;
	private SQLiteDatabase db;

	public StockInServiceDao(Context context) {
	...
	public List<HashMap<String, String>> findListByUpLoadFlag(int upLoadFlag,
		...
		Cursor cursor = db.rawQuery(
				"select * from stockin where upLoadFlag=? and userId=?",
	...
	public List<HashMap<String, String>> findHistory(int userId) {
		...
		List<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();
		Cursor cursor = db.rawQuery(
				"select * from stockin where userId=?",
				new String[] { userId + "" });
		while (cursor.moveToNext()) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("userId",
	...
	public void deleteHistoryById(int id) {
		...
		//Cursor cursor = db.rawQuery("delete from history where id=?",
			//	new String[] { id + "" });
		db.delete("stockin", "id=?", new String[]{id+""});
	}
	...
	public void updateUpLoadFlagHistoryById(int id) {
		...
		cv.put("upLoadFlag", 1+"");
		db.update("stockin", cv, "id = ?", new String[] { id + "" });
	}
}
