...
import com.csei.database.entity.service.UninstallService;

public class UninstallServiceDao implements UninstallService {
	private DBOptions dbOptions;
	private SQLiteDatabase db;

	public UninstallServiceDao(Context context) {
	...
	@Override
	public ArrayList<HashMap<String, String>> findListByUpLoadFlag(int upLoadFlag,
			int userId) {
		// TODO Auto-generated method stub
		ArrayList<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();
		Cursor cursor = db
				.rawQuery(
						"select * from uninstall where upLoadFlag=? and userId=?",
		...
		while (cursor.moveToNext()) {
			...
			map.put("removeMan",
					cursor.getString(cursor.getColumnIndex("removeMan")));
			map.put("removeStatus",
					cursor.getString(cursor.getColumnIndex("removeStatus")));
			map.put("deviceId",
	...
	public List<HashMap<String, String>> findHistory(int userId) {
		...
		Cursor cursor = db.rawQuery(
				"select * from uninstall where userId=?",
				new String[] { userId + "" });
		while (cursor.moveToNext()) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("userId",
	...
	public void deleteHistoryById(int id) {
		...
		//Cursor cursor = db.rawQuery("delete from history where id=?",
			//	new String[] { id + "" });
		db.delete("uninstall", "id=?", new String[]{id+""});
	}
	...
	public void updateUpLoadFlagHistoryById(int id) {
		...
		cv.put("upLoadFlag", 1+"");
		db.update("uninstall", cv, "id = ?", new String[] { id + "" });
	}
}
